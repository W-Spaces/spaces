...
impl AppsState {
    ...
    fn persist(&self, apps: &[SavedApp]) -> Result<(), String> {
        ...
        let json_content = serde_json::to_string_pretty(apps).map_err(|e| e.to_string())?;
        fs::write(&self.data_file, json_content).map_err(|e| e.to_string())
    }
}
...
#[tauri::command]
fn save_app(state: State<AppsState>, app: SavedApp) -> Result<SavedApp, String> {
    let mut apps = state.apps.lock().unwrap();
    let now = Utc::now().to_rfc3339();

    let idx = apps.iter().position(|a| a.id == app.id);
    let saved = if let Some(i) = idx {
        // Update existing
        let mut updated = app;
        updated.last_used = now;
        apps[i] = updated.clone();
        updated
    } else {
        // Create new
        let mut new_app = app;
        new_app.id = if new_app.id.is_empty() {
            Uuid::new_v4().to_string()
        } else {
            new_app.id.clone()
        };
        new_app.last_used = now;
